...
from stiffness_copilot_mujoco.rl.residual_insertion_env import ResidualRRLRewardConfig
...
from stiffness_copilot_mujoco.rollouts.residual_rrl import ResidualRRLEpisodeSummary, run_residual_rrl_episode
from stiffness_copilot_mujoco.scenes import get_scene_spec
from stiffness_copilot_mujoco.sim.scene import ROOT


DEFAULT_OUTPUT_ROOT = ROOT / "artifacts" / "evaluations" / "residual_rrl"


@dataclass(frozen=True)
class ResidualRRLEvaluationResult:
    output_dir: Path
    aggregate: dict[str, dict[str, float | int | list[int]]]


def aggregate_rrl_pareto(
    rows: list,
    *,
    ...
    for name in ("low", "high", "mid_high_baseline", "residual_bc", "residual_rrl"):
        ...
        if name in ("residual_bc", "residual_rrl"):
        ...
        if name == "residual_rrl":
            rrl_rows = [row for row in selected if isinstance(row, ResidualRRLEpisodeSummary)]
            aggregate["mean_bc_deviation"] = float(np.mean([row.mean_bc_deviation for row in rrl_rows]))
            aggregate["max_bc_deviation"] = float(np.max([row.max_bc_deviation for row in rrl_rows]))
            aggregate["mean_action_rate"] = float(np.mean([row.mean_action_rate for row in rrl_rows]))
            aggregate["early_termination_count"] = int(sum(int(row.early_terminated) for row in rrl_rows))
            aggregate["reward_mean"] = float(np.mean([row.reward_mean for row in rrl_rows]))
            aggregate["reward_p05"] = float(np.percentile([row.reward_p05 for row in rrl_rows], 5.0))
...
def evaluate_residual_rrl(
    *,
    scene: str,
    bc_policy_path: Path,
    rrl_policy_path: Path,
    episodes: int = 50,
    seed: int = 0,
    ...
    rrl_policy = ResidualRRLPolicy.load(rrl_policy_path)
    ...
    for episode_id, perturbation in enumerate(perturbations):
        ...
        rrl = run_residual_rrl_episode(
            policy=rrl_policy,
            policy_path=rrl_policy_path,
            bc_policy=bc_policy,
        ...
        rows.append(rrl)
        print(
            f"episode={episode_id} residual_bc={residual.completion_like} residual_rrl={rrl.completion_like} "
            f"bc_force={residual.max_normal_force:.3f} rrl_force={rrl.max_normal_force:.3f}"
        )

    agg = aggregate_rrl_pareto(
        rows,
        ...
        json.dumps(
            {
                ...
                "rrl_policy": str(rrl_policy_path),
    ...
    return ResidualRRLEvaluationResult(output_dir=output_dir, aggregate=agg)


__all__ = ["DEFAULT_OUTPUT_ROOT", "ResidualRRLEvaluationResult", "aggregate_rrl_pareto", "evaluate_residual_rrl"]
